    ...
    def _get_candidates(self):
        ...
        for sat in self.constellation.satellites:
            if self.cga.check_visibility_fast(self.user_pos, sat['pos']):
                # Luôn tính CGA features
                feats_cga = self.cga.to_features(self.user_pos, sat['pos'])
                
                # --- FIX: Tính XYZ features cho Baseline an toàn hơn ---
                # Lấy vector tương đối: Sat - User (Grade 1)
                # self.user_pos(1) trả về vector part của CGA Point
                u_vec = self.user_pos(1)
                s_vec = sat['pos'][1]
                
                # Tính vector hiệu (Difference Vector)
                diff_vec = s_vec - u_vec
                
                # Dùng Inner Product (|) để chiếu lên trục e1, e2, e3
                # Kết quả của (A | B) là multivector vô hướng
                # Truy cập [0] để lấy giá trị float thực sự
